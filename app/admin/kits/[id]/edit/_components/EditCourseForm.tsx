...
import { editKit } from "../actions";
import { AdminKitDetailType } from "@/app/data/admin/admin-get-kit";
interface iAppProps {
  data: AdminKitDetailType;
}

const EditKitForm = ({ data }: iAppProps) => {
  const [isPending, startTransition] = useTransition();
  const router = useRouter();
  const id = data.id;
  const form = useForm<kitSchemaType>({
    ...
    defaultValues: {
      ...
      category: data.category as kitSchemaType["category"],
      smallDescrption: data.smallDescrption,
      slug: data.slug,
      status: data.status as kitSchemaType["status"],
    },
  });
  function onSubmit(values: z.infer<typeof kitSchema>) {
    startTransition(async () => {
      const { data, error } = await tryCatch(editKit(values, id));
      if (error) {
