...
const withDatabaseGuard =
  <T extends (req: NextRequest) => Promise<Response>>(handler: T) =>
  async (req: NextRequest): Promise<Response> => {
    try {
      ...
      if (isDatabaseUnavailable(error)) {
        console.error(
          "[Auth] Database unavailable while handling auth request:",
          error,
        );
...
const guardedHandlers = {
  GET: withDatabaseGuard(authHandlers.GET),
  POST: withDatabaseGuard(authHandlers.POST),
};

export const GET = guardedHandlers.GET;
...
export const POST = async (req: NextRequest) => {
  ...
  return guardedHandlers.POST(req);
};
